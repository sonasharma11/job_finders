...
if (isset($_POST['login'])) {
    ...
    $stmt = $con->prepare("SELECT id, name, password, user FROM tbl_users WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
        $stmt->bind_result($id, $name, $hashed_password, $user);
        $stmt->fetch();
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            if ($user == 'admin') {
                ...
                header("Location: rec_dashboard.php"); // Recruiter dashboard
            } else {
                header("Location: applyforjob.php"); // Normal user (job seeker)
            }
            exit();
        } else {
...
<body>
        ...
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="collapse navbar-collapse " id="navbarNav">
                <ul class="navbar-nav mb-2 ms-auto mb-lg-0">
                    ...
                    <li class="nav-item mx-2">
                        <a class="nav-link active text-dark fw-bold text-secondary hover-success" href="login_process.php">LOGIN</a>
                    </li>
                    <li class="nav-item mx-2">
    ...
    <section class="text-white text-center" style="background-image: url('images/sona.png'); 
        ...
        <div class="container align-items-center">
            <h1 class="fw-light" style="font-size: 3rem;">Login</h1>
            <a href="index.php" class="text-decoration-none text-success bg-light fs-5"> Home </a><span
                class="text-dark bg-light fs-5"> > Login</span>
        </div>
    </section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $con->prepare("SELECT id, name, password FROM tbl_users WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();
        ...
        if ($stmt->num_rows > 0) {
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                if (isset($_SESSION['redirect_to'])) {
                    ...
                    echo "<script>alert('Login successful!'); window.location='applyforjob.php';</script>";
                }
            } else {
                echo "<script>alert('Incorrect password!');</script>";
            }
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <a href="forgot_password.php" class="text-success text-decoration-none fw-semibold">Forgot Password?</a>
                                </div>

                                <div class="d-grid">
                                    <button type="submit" class="btn btn-success btn-lg rounded-3 fw-semibold" name="login">
                                        Login
                                    </button>
                                </div>
                            </form>
                            ...
                            <div class="text-center mt-4">
                                <p class="text-muted mb-0">
                                    ...
                                    <a href="register_process.php" class="text-success fw-semibold text-decoration-none">SIGN UP</a>
                                </p>
                            </div>
                        </div>
                    </div>
